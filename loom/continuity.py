...
class BugItem:
    ...
    offset: int = -1           # 证据句在本章正文体的字符偏移(-1=未定位;正文体不含标题行)
...
**只输出一个 JSON 对象**(不要任何额外文字、不要代码块围栏),结构:
{"除虫报告": [...], "状态入账": [...]}
「除虫报告」无矛盾给空数组 [];有则每条一个对象、按严重度降序、最多 6 条:
{"严重度": 5, "类别": "物品", "问题": "一句话冲突", "本章证据": "原文短引(逐字摘自本章终稿)", "前情证据": "第2章「原文短引或账本行」", "落点": "正文", "修改示例": "一句可直接替换的写法"}
(严重度=1-5 整数;类别只能从 物品/人设/规则/时间/衔接/其他 里选,不许自造;落点只能从 正文/设定 里选——该改设定文件的写「设定」并在修改示例里点名文件;本章证据必须是本章终稿里的逐字连续片段,别改写别概括)
「状态入账」是本章新发生的状态变更行(没有给空数组 []),每行严格四类之一:
- [物品] 实体:变更 | 证据:「原文短引」
- [状态] 人物:新状态(变化原因) | 证据:「原文短引」
- [规则] 规则名:数值/条款 | 证据:「原文短引」
- [时钟] 章末:故事内时间"""

_KINDS = ("物品", "人设", "规则", "时间", "衔接", "其他")

_REPORT_LINE = re.compile(r"^[-·•]\s*(.+)$")


def _clamp_stars(v) -> int:
    """严重度归一到 1-5 整数;给不了数字按 3。"""
    try:
...
def _parse_scan_json(raw: str) -> tuple[list[BugItem], list[str]] | None:
    """JSON 结构化解析(G2)。取首个 { 到末个 } 容错围栏/废话;结构不符返回 None 让调用方降级。"""
    ...
    try:
        ...
        return None
    if not isinstance(data, dict) or "除虫报告" not in data and "状态入账" not in data:
        return None
    report = data.get("除虫报告") or []
    ledger = data.get("状态入账") or []
    if not isinstance(report, list) or not isinstance(ledger, list):
        return None
    items: list[BugItem] = []
    for entry in report:
        if not isinstance(entry, dict) or not str(entry.get("问题") or "").strip():
            continue
        kind = str(entry.get("类别") or "").strip()
        if kind not in _KINDS:
            kind = "其他"     # 枚举外一律归「其他」——类别不许自造( prompt 已禁,这里兜底)
        target = "设定" if "设定" in str(entry.get("落点") or "") else "正文"
        items.append(BugItem(
            _clamp_stars(entry.get("严重度")), kind, str(entry["问题"]).strip(),
            evidence=str(entry.get("本章证据") or "").strip().strip("「」\""),
            prior=str(entry.get("前情证据") or "").strip(),
            target=target,
            fix=str(entry.get("修改示例") or "").strip()))
    state_lines = [l.strip() for l in ledger if isinstance(l, str) and statebook._LINE_RE.match(l.strip())]
    return items, state_lines


def _parse_scan_legacy(raw: str) -> tuple[list[BugItem], list[str]]:
    """旧双段文本格式(===除虫报告===/===状态入账===)宽容解析——老响应/JSON 炸了时降级用。"""
    ...
    for line in report_part.splitlines():
        ...
        if kind not in _KINDS:
            kind = "其他"
        item = BugItem(stars, kind, segs[2])
        for seg in segs[3:]:
...
def parse_scan(raw: str) -> tuple[list[BugItem], list[str]]:
    """JSON 优先(G2),解析不出结构就降级旧双段文本格式。入账段只认 statebook 四类行。"""
    got = _parse_scan_json(raw)
    return got if got is not None else _parse_scan_legacy(raw)


def _fill_offsets(items: list[BugItem], body: str) -> None:
    """给每条 issue 算证据句在本章正文体的字符偏移(前端按建议改定位用);找不到留 -1。"""
    for it in items:
        if it.offset < 0 and it.evidence:
            it.offset = body.find(it.evidence)
...
def scan_chapter(project_root: Path, chapter_n: int, body: str, backend: Backend, *,
    ...
    G5 增量扫描:确定性检测全量(纯 CPU);LLM 检测只用 statebook.snapshot_for 的近窗(默认 8 章)+全部规则行+全部消耗品行。
    卡章纲由 budget.fold_recaps 按窗口裁;硬设定超 2000 字符自动截半;前两章结尾各 1200 字不变。
    """
    ...
    try:
        ...
        if hardfacts.strip():
            # G5: 增量扫描——大书硬设定节选送,最多 2000 字符。全文已在 snapshot_for 的远章消耗/规则行覆盖。
            hf = hardfacts[:2000] if len(hardfacts) > 2000 else hardfacts
            parts.append(f"## 硬设定(关键节选)\n{hf}")
        parts.append("## 你的任务\n按系统要求输出两段:除虫报告 + 状态入账。")
