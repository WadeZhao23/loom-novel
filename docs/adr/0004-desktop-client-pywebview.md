...
打包成**无终端窗口**(`console=False`)的后果:Windows 上进程没有标准输出,`sys.stdout`/`stderr` 为 `None`,启动期任何写 std 流(uvicorn 配日志 `isatty()` 首当其冲)都会崩,Mac `.app` 有有效 stdout 故不触发——入口 `loom_app.py` 须把 None 流兜底,排错与 CI 冒烟门见 [packaging/README](../../packaging/README.md)。
