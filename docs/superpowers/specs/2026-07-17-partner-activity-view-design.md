...
>
> **P1.5「停」真机验(2026-07-18,真 DeepSeek v4-pro,闭合 §10.5 的 GeneratorExit 待验点)**:759ms 中途 abort → 前端瞬间拿回控制 + 「· 已停止 ·」+ 零 error 气泡;abort 后 `partner_new` 立刻 409 `partner_busy`(§10.2 归档锁门禁挡住 worker 收尾窗口);轮锁 ~1-2s 内释放,归档/重发即通,无锁挂死。断连方案成立,回退端点方案未启用。
