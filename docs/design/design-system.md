...
--font-brand: "Loom Kai", "LXGW WenKai Screen", "LXGW WenKai", "Kaiti SC",
              "STKaiti", "KaiTi", "楷体", "Songti SC", serif;  /* 品牌时刻:logo/弹窗标题/章题 */
...
楷体三条纪律:只用于 ≥18px 的品牌时刻(logo/弹窗标题/章题/引语),**每屏至多一处**;只用 `font-weight: 400`(KaiTi 无粗体,伪粗发虚);功能文本(按钮/菜单/正文)永不用楷。

**Loom Kai** 是随包捆绑的霞鹜文楷屏幕阅读版 GB2312 全 6763 字 + ASCII 子集(1.73MB woff2,`loom/webui/fonts/`,`packaging/subset_font.py` 可复跑),三端不再依赖系统 KaiTi。内部字体名改为 "Loom Kai" 以避开 OFL 保留字体名争议;授权全文随包分发于 `fonts/OFL.txt`(© LXGW,OFL 1.1,源:github.com/lxgw/LxgwWenKai-Screen v1.522)。子集外生僻字(如「彧」「嬛」)按栈逐字回退:本机全量文楷 → 系统楷 → 宋。正文 `--font-read` 保持系统栈,**不捆正文字体**。
