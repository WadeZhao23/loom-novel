...
`编辑·篇幅变化` 与 `润色师·篇幅保持` 问的都是**这一棒把稿子推向章目标了还是推离了**
(`stepgraders._toward_target`),不是「这一棒不该改长度」。

原因是后者与产品意图正面冲突:`loom/agents.py:_length_hint` 明确给编辑/润色师**压缩授权**
——「原稿明显超目标就顺手压回来」。旧口径 `1-|1-ratio|` 会因此惩罚它们干正事:2026-08-12
实测,一次让终稿距目标偏差从 0.236 降到 0.142(客观变好)的改动,被判成「润色师·篇幅保持
回归」(p=0.0247)。

新口径:`passed` = 没推离(容 0.02 噪声)或推完仍落在 ±25% 内(与 `len_tolerance` 封顶同口径);
`score` 按推完之后离目标多远算。**它没有被改松**——「已经达标还乱压」「反方向乱扩」仍然扣得动,
`tests/test_eval_stepgraders.py` 有四条钉住。`chapter_target` 是**必传参数**而非默认 0:
默认值会让忘传的调用方静默拿到 `[not-measurable]` 这个白送的 pass,与上面「gating 当排除判据」
同类陷阱。

另:`大纲师·细纲篇幅` 对照 `evalapi.outline_budget(chapter_target)`(按场次现算,不是写死的 450)。
这一项此前**不存在**——旧的 450 既无 grader 也无 warn 覆盖,改成多少都没有信号说它对不对,
于是真机 20/20 全部越线也没人发现。
