...
- `case.json` —— **只读它的 `context` 字段**(四键:`setting` / `characters` /
  `prev_hook` / `chapter_goal`)。**不要打开 `labels` 字段**——那是本 case 的金标答案,
  提前看到等于抄答案,你的标注就失去了作为独立证据的意义。如果你不小心看到了,换一个还没看过
  的 case,并在该 case 的 note 里如实记录「已看过金标,本次标注作废」。
