...
def _sess(project, backend=None, *, outline=True, anchor=None, **over):
    if outline:
...
def _first_run(project, replies):
    ...
    sess = _sess(project, be, anchor=False, gate_rounds=0)
...
def test_全部产物已提交时重跑零调用(project):
    ...
    sess2 = _sess(project, be2, anchor=False, gate_rounds=0)
...
def test_重放把已提交产物放回工作区(project):
    ...
    assert [label for label, _ in sess2.workspace] == [
        "本章设定锚点", "本章初稿", "本章改稿", "本章终稿"]
...
def test_改了人格提示词就从那件产物起重跑(project):
    ...
    assert [l for l, _ in sess2.workspace] == [
        "本章设定锚点", "本章初稿", "本章改稿", "本章终稿"]
...
def test_重放不再跑质量关卡(project):
    ...
    _seed_outline(project)
    # gate_rounds=1:改稿触发质检、终稿触发去AI味,各要一条复审回复
    be = Rec([_submit("本章初稿", _稿), _submit("本章改稿", _稿), "通过",
              _submit("本章终稿", _稿), "通过"])
