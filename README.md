...
- **看得见它有多懂你**:侧栏「**镜台**」一屏——**每章手改量的两条曲线**(改写率=它像不像你 / 增删率=它懂不懂你要什么)、写作指纹的规则与 anchor 原句、它跟你学到的写法。曲线只画你 learn 过的章:没 learn 的 0% 是「还没看」,不是「不用改」,虚高一次这个数就不可信了。
...
### 0.5.1 · 学改法这回真的能学了
上一版把「AI 交的细纲」和「你改成的」两份原文整个丢给模型让它自己比——两份各一千多字、真正的差异
只有一两行,它找不着,于是每次都回「没归纳出反复出现的改法」。这版**先由程序把差异逐行算出来**再交给它。
真机实测(跑三章、每章手改细纲,作者共删 5 处「情绪:」行、2 处「许的承诺」行):改前回「无」,改后当场
归纳出这两条习惯。同一个模型、同一批稿子。(Loom 第二次栽在同一处:0.3.7 发现「让模型自己数字数」不管用,
这次是「让模型自己找差异」不管用。)另修:镜台补上撤销按钮(上一版承诺了却没有入口);「启动自检」把
全局 key 当成没配、还让你去每本书的 `.env` 补一行(而那正是全局 key 要免掉的动作);让伙伴学另外四个人格时
不再回「再写几章再说」这句办不到的话。老书零迁移。

### 0.5.0 · 镜台 + 它开始跟你学改法
侧栏多了「**镜台**」一屏:**改写率**(它像不像你)和**增删率**(它懂不懂你要什么)两条曲线,
加上写作指纹学到了几条规则、它认的 anchor 原句、每个人格跟你学到的写法。曲线只画你 learn 过的章
——没 learn 的不算 0%,那是「还没看」不是「不用改」。另外,你跟伙伴说一句「学一下大纲师的改法」,
它会比对**你实际把 AI 的细纲改成了什么**,蒸成候选卡给你点头;落的是「个人增补」区、基座一个字不动。
三句实话:要从这版之后新写的章开始攒证据(它比对的那份底子是这版才开始留的)、今天只有大纲师能学、
想反悔得手动删掉 `agents/<角色>.md` 里的「## 个人增补」段(撤销按钮 0.5.1 补)。修:自带的 DeepSeek
通道会写着写着交白卷(V4「想」的过程也占额度,想久了正文的额度就没了,**0.4.4 就存在**);
伙伴确认一张卡后只要你中途手打过字就再也不自动往下引。老书零迁移。

### 0.4.4 · 装不上的问题修了 + 编辑不再蒙眼改稿
8 月 12 日 openai 发了 3.0 大版本换掉底层网络库，从那天起**全新安装会直接起不来**，报错还写着「缺少 openai 这个依赖包」（openai 明明装着，照着它装一百遍也没用）。这版新旧网络库都认，报错也改成说人话。已装好的老用户不受影响。另外：编辑这一棒此前**拿不到上一章正文和状态账本**，却被自检清单要求核对「钩子接住没有」「东西还在不在主角手上」，现在给它了；开篇设定锚点也不再超长挤占你的写作指纹（实测 539→382 字）；还修掉两处「要求 agent 做它做不到的事」。

### 0.4.3 · 把除虫真正修好
上一版给除虫加了新本事,这版修掉它里面的四个 bug:它对照的**永远是最早那一章**而不是上一章、一条判据恒成立、同一份稿子跑两次证据还不一样——现在报的位置和证据是准的;某条检查哑了也会明说,不再装作没问题。另外,新建书自带的占位人物卡(「主角 · 未命名」)以前会被**原样塞进给 AI 的写作指令**,凭空多出一个角色,现在不会了;开篇设定锚点也不再超长挤占你的写作指纹(实测 539→382 字)。顺带修了个版本号 bug:之前 0.4.1 / 0.4.2 的安装包自报 0.4.0。

### 0.4.2 · 除虫更火眼金睛
自动「除虫」这版多认两类毛病:**时间倒流**(前面说「三天后」、这章开头又变回「当天」),和**本该闭关 / 被关着的人突然出场**却没交代怎么出来的;还认得出角色的简称、别名,不再因为叫法变了就漏抓或误伤别人。

### 0.4.1 · 提设定不再沉默崩溃
你把一个不存在的落点发给「伙伴」,它当场就告诉你「这个落点不存在,去『看地基』挑有效的」,而不是默默收下、等你点确认时才炸。
