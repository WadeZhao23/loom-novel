...
加上写作指纹现在学到的规则、每个人格跟你学到的写法。曲线只画你 learn 过的章——没 learn 的
不算 0%,那是「还没看」不是「不用改」。另外,你跟伙伴说一句「学一下大纲师的改法」,它会比对
**你实际把 AI 的细纲改成了什么**,蒸成候选卡给你点头;落的是「个人增补」区、基座一个字不动,
随时一键撤销。修:世界观拆成一节一文件时「冰山真相」挡不住,后期反转漏进了写手的指令;
自定义端点接思考型模型时「想」得太久会把正文额度吃光、写出来是空的;伙伴确认一张卡后
只要你中途手打过字就再也不自动往下引。
