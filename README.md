...
> 后端可插拔:DeepSeek(国产、不用梯子,自带 key)/ Claude Code / Codex(复用客户端登录、免 key)/ **OpenAI 兼容(自定义 base_url,接智谱GLM/Moonshot/Qwen/硅基流动等)**,界面里一键切换 + 检测连接。模型是**可下拉可手填** + 「拉取可用模型」实时列当前真实可用的型号(名字怎么变都不过时);DeepSeek 默认 `deepseek-v4-pro`。
